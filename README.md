...
Для сценария ChatGPT Work → Cloud Browser удобнее использовать быстрый режим:

```text
https://YOUR-PROJECT.vercel.app/?quick=1
```

`quick=1` — только настройка интерфейса и не содержит секретов. После однократного входа через `RELAY_KEY` браузер использует защищённую cookie-сессию.

В интерфейсе доступны:

- `Ctrl/Cmd + Enter` для быстрой отправки;
- автоматический фокус на поле сообщения после входа и отправки;
- очистка формы одним нажатием;
- мобильная компоновка;
- локальная история последних отправок.

История сохраняет только время, успешность, количество символов и количество кнопок. Текст сообщений и `RELAY_KEY` в неё не записываются.
